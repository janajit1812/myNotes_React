...
const Notes = (props) => {
  ...
  // Here useEffect hook is used to mount the components for the first time.
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div className='row my-2'>
        ...
        {notes.map((note) => {
          return <DisplayNotes note={note} key={note._id} id={note._id} updateNote={updateNote} showAlert={props.showAlert} title={note.title} description={note.description} />
        })}
      </div>
    </>
  )
